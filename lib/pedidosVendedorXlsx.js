...
const RENOMBRAR = {
  ...
  // El tablero trae una fila "Javier Viglino" idéntica, día por día, a la de
  // "Jose Gil" (mismo vendedor listado dos veces) — duplicaba cada venta al
  // sumar los dos. Javier Viglino dejó de ser vendedor (confirmado con
  // Víctor 13/08/26); de ahora en más esa fila se funde con Jose Gil.
  'JAVIER VIGLINO': 'Jose Gil',
};
...
// Las columnas de resumen (Total, Proyectado, ...) son fórmulas: exceljs
// devuelve { formula, result, ... } en vez de un número plano.
...
// fila de label -> fila de encabezados (Vendedor, fecha, fecha, ..., Total,
// Proyectado, ...) -> filas de datos hasta la fila "Total". "Total" y
// "Proyectado" se buscan por label (no por posición fija) por si alguna hoja
// tiene una columna de más/menos entre medio.
function leerBloque(worksheet, filaLabel) {
  const filaHeader = filaLabel + 1;
  const header = worksheet.getRow(filaHeader);
  const columnasFecha = [];
  let colDespuesFechas = 2;
  for (let c = 2; c <= worksheet.columnCount; c++) {
    const v = header.getCell(c).value;
    if (v instanceof Date) { columnasFecha.push({ col: c, fecha: fechaISO(v) }); colDespuesFechas = c + 1; }
    else if (v == null) continue;
    else break;
  }

  let colProyectado = null;
  for (let c = colDespuesFechas; c <= Math.min(colDespuesFechas + 4, worksheet.columnCount); c++) {
    if (norm(header.getCell(c).value) === 'PROYECTADO') { colProyectado = c; break; }
  }
  ...
  while (r <= worksheet.rowCount) {
    ...
    for (const { col, fecha } of columnasFecha) {
      const valor = valorNumerico(row.getCell(col).value);
      filas.push({ vendedorRaw: nombreRaw, fecha, valor: valor ?? 0 });
    }
    if (colProyectado != null) {
      const proyectado = valorNumerico(row.getCell(colProyectado).value);
      if (proyectado != null) proyecciones.push({ vendedorRaw: nombreRaw, proyectado });
    }
    r += 1;
  }
  return { filas, proyecciones };
}
...
function acumularProyecciones(porClaveMes, sinMapear, proyecciones, mes, campo) {
  for (const { vendedorRaw, proyectado } of proyecciones) {
    const resueltos = resolverFila(vendedorRaw);
    if (resueltos === null) { sinMapear.add(vendedorRaw); continue; }
    if (resueltos.length === 0) continue;
    const valorPorVendedor = proyectado / resueltos.length;
    for (const vendedor of resueltos) {
      const key = `${vendedor}|${mes}`;
      if (!porClaveMes.has(key)) porClaveMes.set(key, { vendedor, mes, proyectado_cantidad: null, proyectado_monto: null });
      porClaveMes.get(key)[campo] = valorPorVendedor;
    }
  }
}

// Devuelve { filas: [{ vendedor, fecha, cantidad, monto_ars }],
...
export async function parsePedidosVendedorXlsx(buffer) {
  ...
  const porClaveMes = new Map();
  const sinMapear = new Set();

  for (const worksheet of workbook.worksheets) {
    const mes = worksheet.name.trim();
    if (!NOMBRE_HOJA.test(mes)) continue;
    ...
    const bloqueCantidad = leerBloque(worksheet, filaCantidad);
    const bloqueMonto = leerBloque(worksheet, filaMonto);

    acumular(porClave, sinMapear, bloqueCantidad.filas, 'cantidad');
    acumular(porClave, sinMapear, bloqueMonto.filas, 'monto_ars');
    acumularProyecciones(porClaveMes, sinMapear, bloqueCantidad.proyecciones, mes, 'proyectado_cantidad');
    acumularProyecciones(porClaveMes, sinMapear, bloqueMonto.proyecciones, mes, 'proyectado_monto');
  }

  return { filas: [...porClave.values()], proyecciones: [...porClaveMes.values()], sinMapear: [...sinMapear] };
}
