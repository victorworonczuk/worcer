...
- Si el CUIT no está en la base, **da de alta el cliente solo** (`origen = 'Alta automática (import ventas)'`, sin segmento — todavía no tiene historial para ubicarlo en A-F) — así una factura de un cliente nuevo no queda como un caso suelto para cargar a mano. **Excepción a propósito**: si ese CUIT tiene más de un `nombre_facturado` distinto en toda la tabla, o es un patrón de dígito repetido (`00000000000`, `11111111111`, etc. — así el sistema de facturación anota ventas sin identificación real, tipo consumidor final), no se crea nada; esa factura queda con `cliente_id` null y aparece en la lista de "no vinculados" para revisar a mano. Sin esta excepción se terminaría creando un solo cliente que mezcla a varias personas distintas que compartieron el mismo CUIT genérico (pasó en los datos reales: un CUIT tenía 11 nombres distintos asociados).
