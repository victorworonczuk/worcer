...
**Criterio de `segmento` y `confianza_dato`** (no está en ningún script — se calculó a mano en la sesión que armó la base original, esto se reconstruyó analizando los datos ya cargados):
- `segmento` sale de los meses sin comprar (`meses_sin_comprar`, calculado sobre la facturación histórica real): **A** = 1 mes, **B** = 2 meses, **C** = 3-4 meses, **D** = 5-8 meses, **E** = 9-18 meses (con historial de compra en 2025-2026), **F** = sin ninguna compra en 2025-2026. Esto era un cálculo único hecho a mano al armar la base — **ahora se recalcula solo** (ver `schema_segmentacion.sql`): un trigger en `facturas` (`AFTER INSERT OR UPDATE OF fecha, cliente_id OR DELETE`) llama a `recalcular_segmento_cliente(cliente_id)`, que mira el `MAX(fecha)` de todas las facturas de ese cliente contra la fecha de hoy y actualiza `segmento`/`meses_sin_comprar`/`ultima_compra`. Funciona sin importar por dónde entre la factura (Cargar factura a mano, Importar ventas, el script de línea de comandos) porque todas pasan por la misma tabla. Simplificación a propósito: el criterio nuevo usa **E = 9+ meses** (sin techo en 18) y **F = nunca tuvo ninguna factura** — el original tenía a F atado específicamente a "sin compra en el año calendario 2025-2026", una ventana que se iba a volver vieja con el tiempo; esto solo se dispara cuando hay un movimiento en `facturas`, no envejece un cliente solo porque pasó el tiempo sin que nadie cargue nada. Tampoco se corrió un recálculo masivo: los clientes que no tuvieron una factura nueva desde este cambio conservan el segmento (con el texto largo tipo "E - Muy frio...") calculado en el import histórico. Como este criterio no se veía en ningún lado del dashboard (nadie sin el contexto de esta conversación podía saber por qué un cliente está en tal segmento), se agregaron dos tooltips nativos (`title`, sin JS extra): uno general con el ícono ℹ️ al lado del header "Segmento" que lista los 6 criterios fijos, y uno por fila sobre el badge de cada cliente con su dato puntual (`segmentoTooltip()` en `app.js`, arma el texto con `meses_sin_comprar` y `ultima_compra` de ese registro).
- `confianza_dato` no depende de si el cliente tiene teléfono/email cargado — es qué tan seguro estaba el buscador de que la empresa encontrada en la web era realmente ese cliente: **alta** = confirmado por CUIT exacto o sitio oficial de la empresa, **media** = aparece en un directorio de empresas (Nosis, D&B, listados municipales) sin confirmación directa, **baja** = la fuente misma queda con dudas explícitas (frases tipo "tentativo, sin confirmar titularidad"), **sin_datos** = se buscó y no se encontró nada, **en blanco** = son los 312 clientes activos (segmentos A-E) a los que nunca se les corrió la búsqueda web (el enriquecimiento original solo cubrió los dormidos, para intentar recuperarlos).

**`clientes`**: datos de la base unificada (CUIT, ubicación, segmento A-F, facturación total 2025-2026, datos de contacto encontrados) más `estado_contacto` (`pendiente` | `contactado` | `recuperado` | `descartado`) — refleja el resultado de la interacción más reciente, se actualiza solo al registrar una en `interacciones`. La columna `notas` sigue existiendo en la base por compatibilidad pero **ya no se usa** desde el dashboard — reemplazada por el historial (ver abajo), porque un campo de texto único se pisaba en cada edición y perdía todo rastro de contactos anteriores. Los ~873 registros originales vienen del Excel; a partir de ahí, cualquiera con sesión puede sumar clientes nuevos con el botón "+ Nuevo cliente" del dashboard — quedan con `segmento` en blanco (todavía no tienen historial de compras que los ubique en A-F) y `origen = 'Alta manual'` para distinguirlos de la carga inicial. También se puede eliminar un cliente ("🗑 Eliminar" bajo el nombre, con confirmación) — pero la base lo bloquea si tiene facturas cargadas (`facturas.cliente_id` es `ON DELETE NO ACTION`), así que no hay forma de borrar por accidente un cliente con historial de ventas real; primero habría que borrar esas facturas. Las interacciones sí se borran en cascada junto con el cliente. La columna "Ubicación" del dashboard es editable directamente en la tabla (`provincia` con un `<select>` de la misma lista fija que usa el filtro y el modal de alta, `localidad` y `domicilio` con texto libre) — guarda solo al cambiar, mismo patrón que el estado de contacto. `rubro` también es editable ahí mismo. Esto sirve para completar a mano los datos que la búsqueda web no encontró para un cliente puntual (caso real: Javier Titano no tenía rubro, domicilio ni descripción). `descripcion` se sacó de su propia columna: ocupaba espacio fijo en la planilla en casi todas las filas vacío. Ahora vive dentro de la columna "Cliente", oculta por defecto — un ícono (📝, marcado con más opacidad si el cliente ya tiene descripción cargada) al lado del nombre abre/cierra un `<textarea>` chico con el mismo campo de siempre (`onToggleDescripcion` en `app.js`, guarda un `Set` de clientes con el panel abierto en `state.openDescripcion`, sin pedirle nada al servidor solo por mostrarlo). El lugar que dejó libre la columna "Descripción" pasó a ser la columna "Vendedor" (`vendedor`, quién es el responsable/ejecutivo de esa venta) con su propio filtro en la barra de arriba, poblado dinámicamente igual que "Provincia" a partir de los valores ya cargados. `vendedor` se edita con un `<select>` de lista fija (`VENDEDORES` en `app.js`: Sergio Nastaskin, Hernán Acosta, Walter Vernola, Alejandro Vernola, Jose Gil, Javier Viglino, Francisco Baez, Martín Argento, Darío Frank, Walter Fogar, Mariano Cabarrus, Sebastián Guerra, Horacio Vostrosky, Víctor W.) más una opción "Otro" que despliega un campo de texto libre debajo — mismo criterio que `rubro` para no perder valores ya cargados que no matchean ninguno de los nombres fijos (aparecen automáticamente como "Otro" con el texto original). `rubro` admite **más de una opción**: en vez de un `<select>` de una sola opción, es un grupo de checkboxes con la misma lista fija (`RUBROS` en `app.js`: Distribuidor, Venta online, Sanitario, Corralón, Ferretería, Otros) — porque hay clientes que son varias cosas a la vez (ferretería y sanitarios, corralón y ferretería, distribuidor y venta online). Se guarda igual que los teléfonos múltiples: una lista separada por comas en la misma columna de texto, sin tabla nueva. El filtro "Rubro" del dashboard matchea si la opción elegida está en la lista del cliente, sin importar cuántas tenga. Al principio, si un cliente tenía en `rubro` texto libre del enriquecimiento original que no coincidía con ninguna de las 6 opciones, aparecía como un checkbox extra ya tildado para no perder el dato — pero ese texto libre en realidad era la descripción del negocio mal separada por comas (ej. "Sanitarios (griferías, artículos sanitarios, instalaciones de agua y gas)" se partía en 3 checkboxes rotos), no una clasificación real, así que se limpió: se hizo un `update` una sola vez dejando en `rubro` sólo los valores que ya coincidían exactamente con las 6 opciones (232 clientes quedaron sin rubro cargado) y `rubroCellHtml()` ya no arma esa lista extra — el texto original de cada uno sigue disponible igual en `descripcion` (que ya lo tenía, prácticamente duplicado, del mismo enriquecimiento). `confianza_dato` también es editable ahora, con el mismo patrón de `<select>` coloreado que el estado de contacto (`onConfianzaChange`, clases `badge-conf-*` reutilizadas de los badges). `telefono`, `whatsapp`, `email` y `web` son editables en la columna "Contacto", cada uno como campo simple sin botón de acción al lado — al principio tenían íconos (☎ llamar, ✉ enviar email) pero se sacaron a pedido porque en los clientes con datos rotos del enriquecimiento original (texto tipo "sin_datos, ver Facebook" en vez de un número real) el ícono quedaba apuntando a un link roto; `💬` (abrir WhatsApp con el mensaje del segmento precargado) es la única acción rápida que queda. `web` antes solo se mostraba como link de texto cuando el cliente ya tenía el dato cargado (bastante inconsistente, aparecía "en azul" nada más en algunos) — ahora es un `<input>` como los demás, visible y editable en todos los clientes tengan o no el dato. `telefono` admite **más de un número**: la columna guarda una lista separada por comas en el mismo campo de texto (sin tabla nueva), con un botón "+ Agregar otro teléfono" para sumar filas antes de guardar — `whatsapp`, `email` y `web` se quedaron como campo único a propósito (no se pidió lo mismo para esos). `canal_captacion` (columna "Cómo llegó", entre Vendedor y Facturación) responde a "cómo llegó este cliente a Worcer" — pensado para que un empleado nuevo pueda ver la planilla y entender el contexto de cada cliente sin tener que preguntarle a nadie. Es un `<select>` de lista fija (`CANALES_CAPTACION` en `app.js`: Contacto telefónico, Visita comercial, Referido, Feria/evento, Redes sociales/Web, Cliente histórico, Otro) con su propio filtro — a diferencia de `vendedor`, acá no hay opción "Otro" con texto libre porque no había datos previos que preservar (columna nueva, arranca todo en blanco). También se puede elegir al cargar un cliente nuevo por "+ Nuevo cliente" (`nc-canal-captacion` en el modal). Ojo, no confundir con `origen`: ese campo es otra cosa completamente distinta, ya existía antes y indica si el cliente estaba activo o histórico al momento de la carga original (`Activo 2025-2026` / `Histórico (sin compras recientes)`) — no tiene que ver con cómo se consiguió el contacto.

También hay un filtro de "Localidad" (poblado dinámicamente igual que "Provincia", a partir de los valores ya cargados) — antes solo se podía filtrar por provincia, que es demasiado ancho para encontrar un cliente puntual. Nota: como la localidad viene de texto libre del import histórico, hay variantes de mayúsculas para el mismo lugar (ej. "ALBERTI" y "Alberti" aparecen como dos opciones separadas en el filtro) — no se normalizó porque no se pidió y afecta a cientos de registros; si hace falta limpiarlo en algún momento, avisar.

La barra de filtros se separó en dos renglones (`.filters-search` para el buscador + contador de resultados, `.filters` para los `<select>`) porque con la cantidad de filtros que se fueron agregando (segmento, provincia, localidad, confianza, estado, rubro, vendedor, canal de captación) el buscador mezclado con los `<select>` en una sola fila `flex-wrap` quedaba desprolijo, partido en dos líneas de forma inconsistente según el ancho de pantalla.
...
**`interacciones`**: historial de contacto con cada cliente (`cliente_id`, `usuario`, `canal` — llamado/whatsapp/email/otro —, `resultado` — contactado/recuperado/descartado —, `nota`, `proximo_seguimiento`, `created_at`). Cada registro es un evento nuevo, nunca se pisa uno anterior. Al guardar una interacción, `clientes.estado_contacto` se actualiza automáticamente para que coincida con el `resultado` más reciente — así los filtros del dashboard siguen funcionando sin que nadie tenga que tocar el estado a mano por separado. `proximo_seguimiento` es una fecha opcional ("volver a contactar el...") que se carga junto con la interacción; la más reciente de cada cliente define su seguimiento vigente. El dashboard la muestra como una etiqueta bajo el botón de historial (en rojo si ya venció) y suma una card "📅 Seguimientos vencidos" — clickeable para filtrar la tabla a solo esos clientes — así queda claro todos los días a quién hay que volver a llamar sin depender de la memoria de cada vendedor. También hay una meta semanal de contactos (`META_CONTACTOS_SEMANAL` en `app.js`, hoy en 50 — se cambia a pedido, no tiene UI de configuración porque es un solo número que no vale la pena convertir en pantalla propia) mostrada como card "🎯 Contactos esta semana": cuenta las `interacciones` con `created_at` desde el lunes de la semana actual (00:00 hora local) hasta ahora, sobre el total del equipo (no está desglosada por vendedor todavía, aunque `interacciones.usuario` ya lo permitiría si hiciera falta más adelante). Muestra "Faltan N" mientras no se llega, y se pone verde con "¡Meta cumplida!" al llegar o superarla — mismo criterio visual que "Seguimientos vencidos" pero en sentido positivo. También es clickeable como esa: filtra la tabla a solo los clientes que tuvieron al menos una interacción desde el lunes, para poder ver exactamente quiénes fueron los contactados que componen el número de la card (sin agregar un dropdown de filtro aparte, mismo patrón que "Seguimientos vencidos"). Se dejó afuera a propósito un "proyectado" de ritmo semanal (cuántos contactos se van a hacer al ritmo actual): agrega complejidad de cálculo (días hábiles restantes, etc.) para poco beneficio sobre simplemente mostrar cuántos faltan; se puede sumar después si el conteo simple no alcanza. Para que ese número sea real y no se infle registrando el mismo cliente varias veces el mismo día, no se puede cargar más de una interacción por cliente por día calendario (hora local): si ya hay una hoy, el formulario se reemplaza por un aviso ("Ya se registró un contacto hoy con este cliente...") en vez del `<textarea>`/selects de siempre. Si fue un error, cada fila del historial tiene un ícono 🗑 para eliminarla (antes no se podía borrar ninguna) — al borrar, `clientes.estado_contacto` se recalcula tomando el `resultado` de la interacción más reciente que quede (o queda en blanco/"Pendiente" si no queda ninguna), y el bloqueo del día se libera para poder cargar una nueva.
...
Filtra `factura_items` por cliente(s), línea, pieza+calidad y rango de fechas. Responde directamente preguntas tipo "¿cuántos inodoros cortos comercial compró tal cliente en julio?". Tiene datos de: facturas cargadas a mano desde `/nueva-factura.html` con piezas completadas, **y** facturas importadas desde los reportes "Salidas de Stocks" del sistema de facturación (ver `scripts/parse-salidas-stock.py` + `scripts/import-salidas-stock.cjs`) — el import histórico de 1.615 facturas es el único que no tiene desglose por pieza.

**Import de detalle por pieza desde "Salidas de Stocks"**: el sistema de facturación (Cerámica/Porcelanas/Presupuesto) exporta un reporte con cantidad y precio por producto y por comprobante, en formato .xlsx real (no el SpreadsheetML de "Importar ventas"). El código de producto (ej. `0011  INODORO CORTO NAPOLES`) **no sigue un esquema numérico 100% estricto** entre líneas — el mismo dígito de "tipo" significa cosas distintas según la línea (ej. el dígito 6 es "lavatorio monocomando" en Nápoles pero no existe como concepto separado en otras líneas) — así que el mapeo se hace por el texto completo del ítem (código+descripción) contra una tabla explícita (`ITEM_A_PIEZA` en `parse-salidas-stock.py`), no decodificando el número. Este reporte trajo productos que no estaban en el catálogo (repuestos/accesorios: tapa de mochila, elemento de mochila, tapa de inodoro — se agregaron como línea `Repuestos`; y algunos ítems sueltos con código genérico `999` como pedidos especiales — línea `Otros`).

- Cruza cada renglón contra la factura ya cargada (por `numero_comprobante` + `tipo_comprobante` + `empresa`) y contra la pieza del catálogo. Si la factura todavía no fue importada por "Importar ventas" (el reporte de salidas de stock cubre todo el año, no solo el mes ya importado), ese renglón queda afuera hasta que se importe el mes correspondiente — no es un error, se reporta en la consola.
...
El botón "✉ Enviar email" se sacó del dashboard (columna Contacto) a pedido, para no tener íconos de acción ahí. El código sigue entero y funcional (`buildEmail`, `EMAIL_TEMPLATES`, `onSendEmail` en `app.js`, más `/api/send-email`) por si se vuelve a enganchar desde otro lugar (por ejemplo un envío por segmento/campaña en lugar de uno por uno) — hoy está sin ningún botón que lo dispare. Como referencia, así funcionaba:
