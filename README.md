...
- Después de insertar, vincula automáticamente cada factura nueva a un `cliente_id` comparando el CUIT (igual que el import histórico). Si el CUIT no está en la base, la factura igual queda cargada — el dashboard va a mostrar ese monto en el próximo cliente que se cree con ese CUIT, o se puede revisar en la lista de "no vinculados" que muestra la pantalla al terminar.
